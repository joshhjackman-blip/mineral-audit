    ...
    <div className="min-h-screen">
      ...
      <nav className="border-b border-[var(--line)] px-6 sm:px-10">
        <div className="mx-auto flex h-16 max-w-[760px] items-center justify-between">
          <Link
            href="/"
            className="font-serif-doc text-[17px] font-semibold text-[var(--ink)] hover:opacity-75"
          >
            ← Mineral Audit
          </Link>
          {step !== "submitted" && (
            <div className="flex items-center gap-3 text-[13px]">
              <StepPip
                active={step === "info"}
                done={["upload", "submitted"].includes(step)}
                label="Lease info"
              />
              <div className="h-px w-6 bg-[var(--line)]" />
              <StepPip
                active={step === "upload"}
                done={["submitted"].includes(step)}
                label="Documents"
              />
            </div>
          )}
      ...
      <div className="mx-auto max-w-[760px] px-6 py-12 sm:px-10">
        ...
        {step === "info" && (
          <form onSubmit={handleInfoSubmit} noValidate>
            <div className="mb-10">
              ...
              <h1 className="font-serif-doc text-[32px] font-semibold text-[var(--ink)]">
                About your lease
              </h1>
              <p className="mt-2 text-[15px] text-[var(--ink-soft)]">
                All of this information appears on your division order. If you
                do not have it handy, locate your division order before continuing.
              </p>
            </div>
              ...
              <Row>
                <Field label="Your full name" required error={errors.ownerName}>
                  <Input
                    ...
                    onChange={(v) => updateInfo("ownerName", v)}
                ...
                <Field label="Email address" required error={errors.ownerEmail}>
                  <Input
                    ...
                    onChange={(v) => updateInfo("ownerEmail", v)}
            ...
            <Section title="Lease details">
              <Row>
                <Field
                  ...
                  hint="Printed on your division order, e.g. 'Smith A Unit'"
                >
                  <Input
                    value={info.leaseName}
                    onChange={(v) => updateInfo("leaseName", v)}
                    placeholder="Smith A Unit"
                  />
                </Field>
                <Field label="Operator name" required error={errors.operatorName}>
                  <Input
                    value={info.operatorName}
                    onChange={(v) => updateInfo("operatorName", v)}
              ...
              <Row>
                <Field label="County" required error={errors.countyName}>
                  <Input
                    value={info.countyName}
                    onChange={(v) => updateInfo("countyName", v)}
                    placeholder="Howard County"
                  />
                </Field>
                <Field
                  label="RRC District"
                  required
                  error={errors.districtNo}
                  hint="2-character code on your division order"
                >
                  <select
              ...
              <Row>
                <Field
                  ...
                  hint="Numeric, e.g. 054321"
                >
                  <Input
                    value={info.leaseNo}
                    onChange={(v) => updateInfo("leaseNo", v)}
                    placeholder="054321"
                    mono
                  />
                </Field>
                <Field
                  label="API number"
                  hint="Optional — 14-digit number on well completion reports"
                >
                  <Input
                    value={info.apiNumber}
                    onChange={(v) => updateInfo("apiNumber", v)}
                    placeholder="42-211-38845-0000"
                    mono
                  />
                </Field>
              </Row>
            ...
            <Section title="Royalty interest">
              <Row>
                <Field
                  ...
                  hint="From your lease, e.g. 1/8, 3/16, 1/5"
                >
                  <Input
                    value={info.royaltyFraction}
                    onChange={(v) => updateInfo("royaltyFraction", v)}
                    placeholder="3/16"
                  />
                </Field>
                <Field
                  label="Decimal interest"
                  hint="From your division order, e.g. 0.03750000"
                >
                  <Input
                    value={info.decimalInterest}
                    onChange={(v) => updateInfo("decimalInterest", v)}
                    placeholder="0.03750000"
                    mono
                  />
                </Field>
              </Row>
              <Row>
                <Field label="Lease execution date" hint="Date the lease was signed">
                  <Input
                    value={info.leaseDate}
                    onChange={(v) => updateInfo("leaseDate", v)}
                    type="date"
                  />
                </Field>
              </Row>
            ...
            <Section title="Audit period">
              <p className="mb-4 text-[14px] text-[var(--ink-soft)]">
                Which months do you want audited? We can review up to 4 years
                back — the Texas statute of limitations on royalty recovery.
              </p>
              <Row>
                <Field label="From">
                  <Input
                    value={info.auditPeriodStart}
                    onChange={(v) => updateInfo("auditPeriodStart", v)}
                    type="month"
                  />
                </Field>
                <Field label="To">
                  <Input
                    value={info.auditPeriodEnd}
                    onChange={(v) => updateInfo("auditPeriodEnd", v)}
                    type="month"
                  />
                </Field>
              </Row>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <button
                type="submit"
                className="rounded-sm bg-[var(--ink)] px-6 py-2.5 text-[13.5px] font-medium text-[var(--paper)] transition-opacity hover:opacity-85"
              >
        ...
        {step === "upload" && (
          <form onSubmit={handleUploadSubmit}>
            <div className="mb-10">
              ...
              <p className="mt-2 text-[15px] text-[var(--ink-soft)]">
                Upload PDFs or images. Scanned documents are fine — legibility
                matters more than scan quality.
              </p>
            </div>

            <div className="mb-6 rounded-sm border border-[var(--line)] bg-[var(--paper-raised)] px-5 py-4 text-[14px] text-[var(--ink-soft)]">
              Auditing:{" "}
              <span className="font-semibold text-[var(--ink)]">{info.leaseName}</span>
            ...
            <div className="flex flex-col gap-4">
              <UploadZone
                label="Oil and gas lease"
                required
                hint="The original lease document containing the royalty clause."
                file={files.lease}
                onChange={(e) => handleFileChange("lease", e)}
                accept=".pdf,.jpg,.jpeg,.png,.tiff"
              />
              <UploadZone
                label="Division order"
                required
                hint="States your decimal interest per well."
                file={files.divisionOrder}
                onChange={(e) => handleFileChange("divisionOrder", e)}
                accept=".pdf,.jpg,.jpeg,.png,.tiff"
              />
              <MultiUploadZone
                label="Check stubs / owner statements"
                required
                hint="Upload one file per month, or a combined PDF. The more months the better."
                files={files.checkStubs}
                onChange={(e) => handleMultiFileChange("checkStubs", e)}
                onRemove={(i) => removeMultiFile("checkStubs", i)}
                accept=".pdf,.jpg,.jpeg,.png,.tiff"
              />
              <MultiUploadZone
                label="1099-MISC tax forms"
                hint="Optional. Useful as a cross-check on annual totals."
                files={files.taxForms}
                onChange={(e) => handleMultiFileChange("taxForms", e)}
                onRemove={(i) => removeMultiFile("taxForms", i)}
            ...
            <div className="mt-8 flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-sm bg-[var(--ink)] px-6 py-2.5 text-[13.5px] font-medium text-[var(--paper)] transition-opacity hover:opacity-85 disabled:cursor-not-allowed disabled:opacity-40"
              >
                Submit for audit
              </button>
            </div>
        ...
        {step === "submitted" && (
          <div className="py-12 text-center">
            <div className="mx-auto mb-6 flex h-14 w-14 items-center justify-center rounded-full border border-[var(--resolved-border)] bg-[var(--resolved-tint)]">
              <span className="text-[24px]" aria-hidden="true">✓</span>
            </div>
            <h1 className="font-serif-doc text-[30px] font-semibold text-[var(--ink)]">
              Documents received
            </h1>
            <p className="mx-auto mt-4 max-w-[480px] text-[15.5px] text-[var(--ink-soft)]">
              We have everything we need to run your audit on the{" "}
              <span className="font-medium text-[var(--ink)]">{info.leaseName}</span> lease.
              We will send your case file to{" "}
              <span className="font-data text-[var(--ink)]">{info.ownerEmail}</span> once
              the analysis is complete.
            </p>
            <div className="mx-auto mt-8 max-w-[480px] rounded-sm border border-[var(--line)] bg-[var(--paper-raised)] p-5 text-left">
              <div className="mb-3 text-[12px] uppercase tracking-wide text-[var(--ink-faint)]">
                What happens next
              </div>
              <div className="flex flex-col gap-3 text-[14px] text-[var(--ink-soft)]">
                <p>We extract and verify your royalty clause from the uploaded lease.</p>
                <p>We request your production history from the Texas Railroad Commission.</p>
                <p>We run the month-by-month variance calculation and flag any discrepancies.</p>
                <p>You receive a case file by email with clear findings and next steps.</p>
              </div>
            </div>
            <Link
              href="/"
              className="mt-8 inline-block text-[13.5px] text-[var(--ink-soft)] hover:text-[var(--ink)]"
            >
              ← Back to home
            </Link>
          </div>
...
function Section({
  ...
  return (
    <div className="mb-8">
      <div className="mb-4 border-b border-[var(--line)] pb-2 text-[12px] font-semibold uppercase tracking-wide text-[var(--ink-faint)]">
        {title}
      </div>
...
function Input({
  ...
  return (
    <input
      ...
      className={`w-full rounded-sm border border-[var(--line-strong)] bg-[var(--paper)] px-3 py-2.5 text-[14px] text-[var(--ink)] placeholder:text-[var(--ink-faint)] focus:border-[var(--ink)] focus:outline-none ${
        mono ? "font-data" : ""
      }`}
    />
  );
  ...
  return (
    <div className="rounded-sm border border-[var(--line)] bg-[var(--paper-raised)] p-5">
      <div className="mb-1 flex items-center gap-2">
        ...
        {required && (
          <span className="rounded-sm bg-[var(--ink)] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--paper)]">
            Required
          </span>
        )}
      </div>
      ...
      {files.length > 0 && (
        <div className="mb-3 flex flex-col gap-1.5">
          {files.map((f, i) => (
            <div
              key={i}
              className="flex items-center gap-3 rounded-sm border border-[var(--resolved-border)] bg-[var(--resolved-tint)] px-3 py-2"
            >
              <span className="text-[13px] font-medium text-[var(--resolved)]">✓</span>
              ...
              <button
                type="button"
                onClick={() => onRemove(i)}
                className="text-[12px] text-[var(--ink-faint)] hover:text-[var(--alert)]"
              >
                Remove
              </button>
            </div>
      ...
      <label className="flex cursor-pointer items-center justify-center rounded-sm border border-dashed border-[var(--line-strong)] px-4 py-5 text-center transition-colors hover:border-[var(--ink)]">
        <div>
          ...
          <p className="mt-1 text-[12px] text-[var(--ink-faint)]">
            PDF, JPG, PNG, TIFF — multiple files allowed
          </p>
        </div>
        <input
          type="file"
